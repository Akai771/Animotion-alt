...
const AnimePlayer: React.FC = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${import.meta.env.VITE_API}/api/v2/hianime/episode/servers?animeEpisodeId=${selectedEpisode}`
      )
    ...
    axios
      .get(
        `${import.meta.env.VITE_API}/api/v2/hianime/episode/sources?animeEpisodeId=${selectedEpisode}&server=${server}&category=${format}`
      )
      .then((res) => {
        ...
        <Card className="w-full p-5">
          ...
          <Card className="flex flex-row items-center justify-between p-2 mt-3">
            ...
            <div className="flex flex-row gap-5">
              ...
              <Card className="flex flex-row gap-10 p-2 bg-[--bgColor3]">
                <div className="flex flex-col items-start">
                  <span className="text-xs font-light text-neutral-500">Server:</span>
                  <div className="flex flex-row items-center gap-2">
                    <Button
                      variant="outline"
                      onClick={() => setServer("hd-1")}
                      className={`${server === "hd-1" ? "bg-[--primary-color] text-white" : ""}`}
                    >
                    < Server size={16}/>HD-1
                    </Button>
                    <Button
                      ...
                      className={`${server === "hd-2" ? "bg-[--primary-color] text-white" : ""}`}
                    >
                    < Server size={16}/>HD-2
                    </Button>
                  </div>
                ...
                <div className="flex flex-col items-start">
                  <span className="text-xs font-light text-neutral-500">Formats:</span>
                  <div className="flex flex-row items-center gap-2">
                    <Button
                      variant="outline"
                      onClick={() => setFormat("sub")}
                      className={`${format === "sub" ? "bg-[--primary-color] text-white" : ""}`}
                    >
                    ...
                    <Button
                      ...
                      className={`${format === "dub" ? "bg-[--primary-color] text-white" : ""}`}
                    >
                      <Mic size={16} /> Dub
                    </Button>
                  </div>
            ...
            <div className="flex flex-row gap-5">
              <div className="border border-l-1" />
              <div className="flex flex-col items-start">
                <span className="text-xs font-light text-neutral-500">Navigation:</span>
